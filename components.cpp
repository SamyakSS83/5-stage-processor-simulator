    ...
    void instruction_fetch() {
        ...
        if (id_needs_flush) {
            DEBUG_PRINT("  Branch taken detected - updating PC to 0x" << std::hex 
                        << pending_branch_target << std::dec << " and invalidating IF/ID");
    ...
    void instruction_decode() {
        ...
        if (!needs_stall && branch_cond != BranchCond::FALSE) {
            ...
            if (cond_met) {
                ...
                id_needs_flush = true;
                DEBUG_PRINT("  Branch target set to 0x" << std::hex << pending_branch_target << std::dec);
            }
        }
...
private:
    ...
    bool stall_just_resolved;  // Add this new flag
};
